...
export function Scene2() {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setPhase(2), 900),   // phone + upload zone appear
      setTimeout(() => setPhase(3), 1700),  // photo 1 lands
      setTimeout(() => setPhase(4), 2300),  // photo 2 lands
      setTimeout(() => setPhase(5), 2900),  // photo 3 lands
      setTimeout(() => setPhase(6), 3800),  // screen flips to details list
      setTimeout(() => setPhase(7), 4600),  // row 1 name + price fill
      setTimeout(() => setPhase(8), 5400),  // row 2 name + price fill
      setTimeout(() => setPhase(9), 6200),  // row 3 name + price fill
      setTimeout(() => setPhase(10), 7000), // arrow + catalog result
    ];
    ...
    >
      ...
      >
        <h2 className="text-[4.8vw] font-black text-[#C41230] font-display leading-tight">
          Upload all your photos at once.
        </h2>
        <p className="text-[2.4vw] text-[#0D0D0D]/65 font-semibold mt-[0.8vh]">
          Then add the name and price for each item.{' '}
          <span className="text-[#C41230]">CatalogKit builds your catalog.</span>
        </p>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 bg-[#F8F8F8] overflow-hidden relative">
            ...
            <AnimatePresence>
              {phase < 6 && (
                ...
                >
                  ...
                  {/* Photo grid — 3 thumbnails land one by one */}
            ...
            {/* === SCREEN B: Details list (name + price per item) === */}
            <AnimatePresence>
              {phase >= 6 && (
                <motion.div
                  key="details"
